...
function plugin_customfields_make_system_name($str)
{
   ...
   return strtr($str, ' ABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()+={}[]<>,.?/~`|:;"\'\\', '_abcdefghijklmnopqrstuvwxyz______________________________');
}
...
function plugin_customfields_showAssociated($item, $withtemplate = '')
{
   ...
   if ($number != 1) { // No data found, so make a link to activate custom fields for this device
      if ($item->canCreate() && $withtemplate != 2) {
         ...
         echo '<a href="' . GLPI_ROOT . '/plugins/customfields/front/itemtype.form.php?itemtype=' . $type . '&amp;id=' . $ID . '&amp;add=add">' . $LANG['plugin_customfields']['Activate_Custom_Fields'] . '</a>';
         echo '</div><br>';
      }
      ...
      echo '<form action="' . GLPI_ROOT . '/plugins/customfields/front/itemtype.form.php" ' . 'method="post" name="form_cf">';
      ...
      while ($fields = $DB->fetch_array($result)) {
         ...
         if ($fields['data_type'] == 'sectionhead' || $fields['data_type'] == 'notes' || $fields['data_type'] == 'text') { // */ // comment out this line if you don't want 'Text' (comment) fields to span both columns
      ...
      if (($count >= 1) && $item->canCreate()) {
         ...
         if (!CUSTOMFIELDS_AUTOACTIVATE) { // Must show the delete button if autoactivate is off
            ...
            echo '<div class="center">' . '<input type="submit" class="submit" value="' . 
               _sx('button', 'Delete permanently') . '" name="delete"/> <b>' . 
               $LANG['plugin_customfields']['delete_warning'] . '</b>' . '</div></td>';
         }
         echo '</tr>';
      }
