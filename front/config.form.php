...
if ($plugin->isActivated("customfields")) {
   ...
   Html::header(__('Configuration'), $_SERVER['PHP_SELF'], 'plugins', 'customfields');
   ...
   while ($data = $DB->fetch_assoc($result)) {
      if (class_exists($data['itemtype'])) {
         ...
         if ($item->canCreate()) {
            ...
            echo "<td><a href='./manage.php?itemtype=" . $data['itemtype'] . "'>" . call_user_func(array(
               $data['itemtype'],
               'getTypeName'
            )) . "</a></td>";
            if ($data['enabled'] == 1) {
   ...
   echo "<a href='./dropdown.php'>" . $LANG['plugin_customfields']['Manage_Custom_Dropdowns'] . "</a>";
   echo "</td></tr>";
   echo "</table></div>";
